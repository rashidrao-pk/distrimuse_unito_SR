...
def _extract_mask_contours(mask_gray, frame_shape_hw):
    """
    Returns contours in original frame coordinates.
    """
...
def create_union_mask(area_inputs, frame_shape_hw):
    """
    Builds one binary mask containing all safety areas.
    area_inputs[area]["mask_bin"] is expected in original frame coordinates.
    """
    h, w = frame_shape_hw
    union_mask = np.zeros((h, w), dtype=np.uint8)

    for _, info in area_inputs.items():
...
def draw_model_input_panel(frame_bgr, area_inputs, latest_results, width=1400, height=900):
    canvas = np.zeros((height, width, 3), dtype=np.uint8)
    canvas[:] = (30, 30, 30)
    ...
    cv2.putText(canvas, "Original Frame + Safety Area Shapes", (20, 35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (230, 230, 230), 2, cv2.LINE_AA)

    frame_panel_x1 = 20
    frame_panel_y1 = 60
    frame_panel_x2 = left_w - 20
    frame_panel_y2 = height - 20

    disp_h = frame_panel_y2 - frame_panel_y1
    disp_w = frame_panel_x2 - frame_panel_x1
    ...
    # -------------------------------------------------
    # Build blurred + faded background outside safe area
    # -------------------------------------------------
    if len(area_inputs) > 0:
        union_mask = create_union_mask(area_inputs, frame_bgr.shape[:2])

        blurred = cv2.GaussianBlur(frame_bgr, (31, 31), 0)

        darkened = (blurred.astype(np.float32) * 0.35).clip(0, 255).astype(np.uint8)

        foreground = frame_bgr.copy()
        background = darkened.copy()

        union_mask_3 = cv2.cvtColor(union_mask, cv2.COLOR_GRAY2BGR)
        frame_vis = np.where(union_mask_3 > 0, foreground, background)
    else:
        frame_vis = frame_bgr.copy()

    frame_disp = cv2.resize(frame_vis, (new_w, new_h), interpolation=cv2.INTER_AREA)

    x_off = frame_panel_x1 + (disp_w - new_w) // 2
    y_off = frame_panel_y1 + (disp_h - new_h) // 2
    canvas[y_off:y_off + new_h, x_off:x_off + new_w] = frame_disp
    ...
    for area_name, info in area_inputs.items():
        ...
        if len(scaled_contours) > 0:
            cv2.drawContours(canvas, scaled_contours, -1, color, 2)

            pt = scaled_contours[0][0][0]
            tx, ty = int(pt[0]), int(pt[1])

            label = area_name
            if "norm_score" in rr:
                label += f" | {rr['norm_score']:.3f}"
            if "status" in rr:
                label += f" | {rr['status']}"

            cv2.putText(canvas, label, (tx, max(20, ty - 8)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)

    # -------------------------------------------------
    # Right side: model inputs
    # -------------------------------------------------
    cv2.putText(canvas, "Input Given to Model (cropped + resized to 128x128)", (left_w + 20, 35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.75, (230, 230, 230), 2, cv2.LINE_AA)

    areas = list(area_inputs.keys())
    n = len(areas)

    if n == 0:
        cv2.putText(canvas, "No valid area inputs yet.", (left_w + 30, 100),
    ...
    for idx, area_name in enumerate(areas):
        ...
        x1 = panel_x1 + c * cell_w
        y1 = panel_y1 + r * cell_h
        x2 = x1 + cell_w - 10
        y2 = y1 + cell_h - 10
        ...
        title = area_name
        if "norm_score" in rr:
            title += f" | norm={rr['norm_score']:.3f}"
        if "status" in rr:
            title += f" | {rr['status']}"

        cv2.putText(canvas, title, (x1 + 10, y1 + 25),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1, cv2.LINE_AA)

        if resized is not None:
            preview_top = y1 + 40
            preview_h = (y2 - preview_top) - 30
            preview_w = (x2 - x1) - 20

            target_h = max(1, preview_h)
            target_w = max(1, preview_w)

            resized_big = cv2.resize(resized, (target_w, target_h), interpolation=cv2.INTER_NEAREST)
            canvas[preview_top:preview_top + target_h, x1 + 10:x1 + 10 + target_w] = resized_big

            cv2.putText(canvas, "128x128 model input", (x1 + 10, y2 - 10),
    ...
    def process_latest_frame(self):
        ...
        try:
            ...
            if self.args.show_model_input:
                input_panel = draw_model_input_panel(
                    frame_bgr,
                    area_inputs,
                    self.latest_results,
                ...
                cv2.imshow("ADVIS Model Inputs", input_panel)
...
def parse_args():
    ...
    p.add_argument("--model_input_width", type=int, default=1400)
    p.add_argument("--model_input_height", type=int, default=900)
